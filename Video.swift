...
class Video {
        ...
        else
        {
            ...
            if let constPictures = pictures
            {
                    ...
                    
                {
                    ...
                    {
                        let redux = constSizes.count - (constSizes.count - 3)
    ...
    func convertToURINumber(uriString: String) -> Int
    {
        let arrayFromURI = uriString.componentsSeparatedByString("/")
    ...
    static func makeVideoFromRlmObjct(videoOnDisk: VideoServiceRlm) -> Video?
    {
        
        if let convertDict: [String: AnyObject] = ["uri": videoOnDisk.uri!,
                                                   "name": videoOnDisk.name!,
                                                   "link": videoOnDisk.videoLink!,
                                                   "description": videoOnDisk.descript!,
                                                   "picturesRlm": videoOnDisk.imageURLString!,
                                                   "tagsRlm": videoOnDisk.tagForAudioRef!,
                                                   "filesRlm": videoOnDisk.m3u8file!]
        {
    ...
    static func makeAudioFromRlmObjct(videoOnDisk: SermonAudioRlm) -> Video?
    {
    ...
    static func makeDownloadAudioFromRlmObjct(videoOnDisk: DownloadAudioRlm) -> Video?
    {
